...
// void draw_cross(uint32_t *pixels, int w_pixels, int h_pixels, int ld_pixels, int i, int j, uint32_t color)
// {
//     const int w_cross = 5;
...
//         pixels[i * ld_pixels + jj] = color;
//     for(int ii = imin; ii < imax; ii++)
//         pixels[ii * ld_pixels + j] = color;
// }
...
struct pcolor_state pcolor_state_alloc(uint32_t *pixels, int w_pixels, int h_pixels)
{
    ...
    state.pixel_color_id = realloc(state.pixel_color_id, w_pixels*h_pixels*sizeof(int));
        ...
        for(int j = 0; j < state->w_pixels; j++)
        {
            ...
            switch (state->pixel_type[id_color]){
                ...
                case PCOLOR_TYPE_EDGE:
                    state->pixels[id_pixels] = (state->show_edge) ? state->color_egde : 
                                    (state->pixel_color_id[id_color] != -1) ? color_grid[state->pixel_color_id[id_color]] : 
                                        state->color_outside;
                    break;
